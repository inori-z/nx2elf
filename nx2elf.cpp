...
struct NsoFile {
  ...
  template <typename T> char *FormatBytes(char *p, T d) {
    for (auto &b : d)
      p += sprintf(p, "%02x", b);
    return p;
  }
...
#undef DT_ASSIGN_U64
    ...
    if (file_type == kMod) {
      ...
      if (dyn_info.symtab >= dyn_info.strtab) {
        fputs("error: raw MOD requires .dynstr directly after .dynsym. please "
              "report this.\n",
              stderr);
        return false;
      }
      ...
      if (seen_shndx.size() != kNumSegment + 1) {
        fputs("error: raw MOD failed to find .data in .dynsym. please report "
              "this.\n",
              stderr);
        return false;
      }
...
#undef ALLOC_SHDR_IF
    ...
    if (eh.MeasureFrame(
            ...
            &eh_frame_ptr, (unsigned long *)&eh_info.frame_size)) {
      eh_info.frame_addr =
